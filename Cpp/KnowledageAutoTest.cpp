...
//改变UI测试
//核心逻辑：Websocket的连接 处理解析Websocket收到的指令进行接口调用
//额外的功能---外部渲染 做下载 上传 h264 编解码实现
//怎么自动化--写好用例 python 批量或者单次的发送上去的 注册事件处理函数，在需要发送消息时触发相应的事件处理函数
//确保demo的稳定 UI是基本不会变化的

//storage模块 销毁的正确性
//统一了C++版本全端共用了 可以单独的打包给出 跨团队使用
